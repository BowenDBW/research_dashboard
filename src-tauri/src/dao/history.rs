...
pub fn log_action(conn: &DbConnection, article_id: i64, action_type: &str) -> Result<(), String> {
    println!("[DEBUG] log_action called: article_id={}, action_type={}", article_id, action_type);
    conn.execute(
        "INSERT INTO user_action_logs (article_id, action_type) VALUES (?, ?)",
        params![article_id, action_type]
    ).map_err(|e| {
        println!("[DEBUG] log_action failed: {}", e);
        format!("记录操作失败: {}", e)
    })?;

    println!("[DEBUG] log_action success");
    Ok(())
}

/// Delete the most recent action log for a specific article and action type
/// Only deletes if created within the last 30 seconds (to avoid deleting old records)
pub fn delete_recent_action(conn: &DbConnection, article_id: i64, action_type: &str) -> Result<(), String> {
    // Only delete if created within last 30 seconds
    let sql = "DELETE FROM user_action_logs
               WHERE article_id = ? AND action_type = ?
               AND created_at >= datetime('now', '-30 seconds')
               ORDER BY created_at DESC LIMIT 1";

    conn.execute(sql, params![article_id, action_type])
