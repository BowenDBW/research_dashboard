...
/// Get today's quick stats (for sidebar)
pub fn get_today_stats(conn: &DbConnection) -> Result<(i64, i64, i64, i64), String> {
    ...
    let today_count: i64 = conn.query_row(
        "SELECT COUNT(*) FROM user_action_logs WHERE DATE(created_at) = ?",
        params![today],
        |row| row.get(0)
...
fn get_reading_stats(conn: &DbConnection, start_date: &str, end_date: &str) -> Result<ReadingStats, String> {
    ...
    let week_count: i64 = conn.query_row(
        "SELECT COUNT(*) FROM user_action_logs WHERE created_at >= DATE('now', '-7 days')",
        [],
        |row| row.get(0)
    ).map_err(|e| format!("查询本周阅读数失败: {}", e))?;

    // Month count (in date range)
    let month_count: i64 = conn.query_row(
        "SELECT COUNT(*) FROM user_action_logs WHERE created_at >= ? AND created_at <= ?",
...
/// Get weekly hour data (day of week x hour)
...
/// Get daily hour data (for heatmap)
...
fn get_domain_distribution(conn: &DbConnection, start_date: &str, end_date: &str) -> Result<Vec<DomainDistribution>, String> {
    ...
    let total_sql = "SELECT COUNT(*) FROM user_action_logs
                     WHERE created_at >= ? AND created_at <= ?";
...
/// Get heatmap data (daily counts with levels)
